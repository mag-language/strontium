...
use crate::machine::{BitwiseMethod, Opcode};
...
pub enum BytecodeError {
    ...
    UnexpectedBytes { expected: Vec<u8>, actual: Vec<u8> }, // Unexpected bytes encountered
...
impl std::fmt::Display for BytecodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BytecodeError::UnexpectedEof(pos) => {
                write!(f, "Unexpected end of bytecode at position {}", pos)
            }
            BytecodeError::UnexpectedBytes { expected, actual } => write!(
                f,
                "Unexpected bytes: expected {:?}, got {:?}",
                expected, actual
            ),
            ...
            BytecodeError::RegisterNotFound(register) => {
                write!(f, "Register not found: {}", register)
            }
